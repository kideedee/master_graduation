...
Để đánh giá đóng góp của kiến trúc chuyên biệt cho tác vụ, học viên thực hiện nghiên cứu loại bỏ thành phần nhằm so sánh bốn cấu hình: (1)~đường cơ sở DNABERT-2, tức mô hình DNABERT-2 chỉ với đầu phân loại tuyến tính đơn giản, không tích hợp mô-đun MKC; (2)~PhaBERT, kiến trúc đề xuất đầy đủ với nhánh CNN đa cửa sổ kết hợp gộp trung bình có mặt nạ toàn cục; (3)~DNABERT-2-CNN, biến thể chỉ sử dụng nhánh CNN đa cửa sổ mà không có nhánh gộp toàn cục; và (4)~DNABERT-2-AP, biến thể thay thế toàn bộ mô-đun MKC bằng cơ chế gộp chú ý. So sánh đường cơ sở DNABERT-2 với PhaBERT cho phép đánh giá đóng góp tổng thể của mô-đun MKC, trong khi so sánh DNABERT-2-CNN và DNABERT-2-AP với đường cơ sở cho phép tách biệt đóng góp riêng của từng thành phần. Bảng~\ref{tab:ablation_results} trình bày kết quả so sánh chi tiết và Hình~\ref{fig:dnabert2_vs_phabertcnn} minh họa trực quan kết quả này.

\begin{table}[h]
    \centering
    \caption{Nghiên cứu loại bỏ thành phần: so sánh DNABERT-2, PhaBERT, DNABERT-2-CNN và DNABERT-2-AP trên bốn nhóm độ dài contig}
    ...
    \begin{tabular}{rclllll}
        ...
        \multirow{3}{*}{PhaBERT}         & Sn  & 82,54 & 88,20 & 90,92 & \textbf{92,31} \\
                                         & Sp  & \textbf{81,30} & 84,39 & \textbf{86,58} & 87,71 \\
                                         & Acc & 82,26 & 87,38 & 90,01 & \textbf{91,34} \\ \hline
        ...
        \multirow{3}{*}{DNABERT-2-AP}    & Sn  & \textbf{85,12} & \textbf{89,04} & 90,84 & 91,72 \\
...
\begin{figure}[h]
    ...
    \caption{So sánh hiệu suất của DNABERT-2, PhaBERT, DNABERT-2-CNN và DNABERT-2-AP trên bốn nhóm độ dài contig}
    \label{fig:dnabert2_vs_phabertcnn}
\end{figure}

Cả ba biến thể (PhaBERT, DNABERT-2-CNN, DNABERT-2-AP) đều vượt trội so với đường cơ sở DNABERT-2 một cách nhất quán trên tất cả các nhóm độ dài, xác nhận rằng các thành phần kiến trúc bổ sung mang lại cải thiện có ý nghĩa so với việc chỉ sử dụng đầu phân loại tuyến tính. So với đường cơ sở DNABERT-2, PhaBERT cải thiện độ chính xác lần lượt 0,74\% (Nhóm A), 1,82\% (Nhóm B), 2,76\% (Nhóm C) và 2,98\% (Nhóm D), với mức cải thiện tăng dần theo độ dài contig. Đặc biệt, độ đặc hiệu được cải thiện đáng kể: tăng 8,05\% ở Nhóm A (từ 73,25\% lên 81,30\%) và 5,12\% ở Nhóm B (từ 79,27\% lên 84,39\%), cho thấy mô-đun MKC nâng cao đáng kể khả năng nhận diện thực khuẩn thể ôn hòa. Xu hướng cải thiện tăng dần theo độ dài phản ánh đặc tính của nhánh CNN đa cửa sổ với ba kích thước bộ lọc (3, 5, 7) theo thiết kế kế thừa từ TextCNN~\cite{kim2014convolutional}: khi chuỗi dài hơn, các mô-típ chức năng đặc trưng của hệ gen thực khuẩn thể trở nên hoàn chỉnh và phong phú hơn, cho phép các bộ lọc tích chập ở nhiều tỷ lệ không gian khai thác hiệu quả hơn các mẫu cục bộ.

So sánh hai thành phần riêng lẻ cho thấy sự khác biệt đáng chú ý giữa DNABERT-2-CNN (chỉ nhánh CNN đa cửa sổ) và DNABERT-2-AP (chỉ gộp chú ý). DNABERT-2-AP đạt độ chính xác cao hơn ở Nhóm A (83,54\% so với 82,18\%) và Nhóm B (87,63\% so với 87,36\%), trong khi DNABERT-2-CNN nhỉnh hơn ở Nhóm C (90,16\% so với 89,88\%) và hai biến thể đạt kết quả gần tương đương ở Nhóm D (91,07\% so với 91,08\%). Xu hướng này cho thấy gộp chú ý mang lại lợi thế trên các contig ngắn, nơi việc tập trung vào các vùng có tính thông tin cao giúp bù đắp cho lượng thông tin trình tự hạn chế, trong khi CNN đa cửa sổ phát huy hiệu quả hơn trên các contig dài nhờ khả năng khai thác các mô-típ chức năng hoàn chỉnh hơn.

PhaBERT, kết hợp cả hai nhánh CNN đa cửa sổ và gộp trung bình có mặt nạ, thể hiện ưu thế rõ rệt trên contig dài: đạt độ chính xác cao nhất ở Nhóm D (91,34\%) cùng độ nhạy vượt trội (92,31\% so với 91,75\% của DNABERT-2-CNN và 91,72\% của DNABERT-2-AP). Đồng thời, PhaBERT đạt độ đặc hiệu cao nhất trong các cấu hình ablation ở Nhóm A (81,30\%) và Nhóm C (86,58\%), cho thấy sự kết hợp hai nhánh cải thiện khả năng nhận diện thực khuẩn thể ôn hòa. Nhìn tổng thể, việc kết hợp trích xuất đặc trưng cục bộ đa tỷ lệ từ CNN với tổng hợp thông tin toàn cục từ gộp trung bình có mặt nạ mang lại hiệu suất cân bằng và ổn định hơn so với từng thành phần riêng lẻ trên toàn phổ độ dài contig.
...
phân tầng 5 phần trên bốn nhóm độ dài contig}
    ...
    \begin{tabular}{rclllll}
        ...
        \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}Nhóm C\\ (800--1.200 bp)\end{tabular}} &
        \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}Nhóm D\\ (1.200--1.800 bp)\end{tabular}} \\ \hline
        \multirow{3}{*}{PhaBERT} & Sn  & \textbf{82,54} & \textbf{88,20} & \textbf{90,92} & \textbf{92,31} \\
