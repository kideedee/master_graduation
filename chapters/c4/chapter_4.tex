...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/error_confusion_matrix.png}
...
Kết quả cho thấy lỗi phân bố không đồng đều giữa hai lớp. Trên cả bốn nhóm độ dài, FPR luôn cao hơn FNR, nghĩa là thực khuẩn thể ôn hòa, lớp thiểu số chiếm khoảng 21\% tập dữ liệu, là lớp bị phân loại sai nhiều hơn. Đây là hệ quả trực tiếp của việc mô hình học thiên về lớp đa số (virulent). Đáng chú ý, khoảng cách giữa hai tỉ lệ lỗi nới rộng theo độ dài contig: ở Nhóm A hai tỉ lệ gần tương đương (18,19\% so với 17,93\%), nhưng đến Nhóm D, FPR (12,43\%) cao gần gấp 1,6 lần FNR (7,62\%). Nói cách khác, khi contig đủ dài để mô hình phân loại chính xác hơn về tổng thể, phần lỗi còn lại có xu hướng dồn vào việc nhầm thực khuẩn thể ôn hòa thành độc lực. Hệ quả thực tiễn là nếu bài toán quan tâm đến việc phát hiện thực khuẩn thể ôn hòa, vốn tiềm ẩn rủi ro trong liệu pháp phage do khả năng biến đổi tiềm tan~\cite{feiner2015new}, thì cần lưu ý mô hình hiện đang phân loại sai nhóm này nhiều hơn.

\subsection{Phân tích lỗi theo thành phần GC}
\label{sec:loi_theo_gc}

Bên cạnh độ dài, học viên khảo sát ảnh hưởng của thành phần GC (tỉ lệ phần trăm base G và C trong contig) đến tỉ lệ lỗi, một chiều phân tích chưa được các công cụ phân loại thực khuẩn thể hiện hành như DeePhage~\cite{wu2021deephage} và PhaTYP~\cite{shang2023phatyp} báo cáo. Các contig được chia thành bốn khoảng GC cố định theo sinh học: dưới 35\%, 35--45\%, 45--55\% và trên 55\%. Hình~\ref{fig:error_heatmap_gc} trình bày tỉ lệ lỗi theo từng khoảng GC trên cả bốn nhóm độ dài dưới dạng bản đồ nhiệt hai chiều độ dài $\times$ GC.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/error_heatmap_length_gc.png}
...
Tỉ lệ lỗi thể hiện quan hệ hình chữ U rõ rệt theo thành phần GC: mô hình mạnh nhất ở khoảng GC trung bình (35--55\%) và yếu hẳn ở hai cực, đặc biệt ở khoảng GC cao (trên 55\%). Trên toàn cục, tỉ lệ lỗi ở khoảng GC trên 55\% lên đến 24,83\%, cao hơn gấp đôi so với mức 10,59\% ở khoảng 45--55\%; ở các contig ngắn (Nhóm A), con số này còn lên đến 28,86\%. Ở cực còn lại, khoảng GC dưới 35\% cũng có tỉ lệ lỗi toàn cục tăng lên 19,20\%, cao hơn rõ rệt so với mức nền khoảng 10--11\% ở vùng GC trung bình, xác nhận cả hai nhánh của đường cong chữ U. Quan trọng hơn, mẫu hình chữ U này lặp lại nhất quán trên cả bốn nhóm độ dài (Hình~\ref{fig:error_heatmap_gc}), cho thấy đây là một đặc tính cố hữu của mô hình chứ không phải nhiễu thống kê do kích thước mẫu. Phát hiện này có ý nghĩa đóng góp: nó chỉ ra một điểm yếu chưa được báo cáo trong các nghiên cứu trước, đồng thời gợi mở rằng các trình tự có thành phần GC cực trị, thường gặp ở một số nhóm vi khuẩn chủ đặc thù, cần được quan tâm đặc biệt khi triển khai mô hình. Một lưu ý kỹ thuật khi diễn giải kết quả là ở các contig ngắn (100--400 bp), ước lượng thành phần GC có sai số lấy mẫu lớn hơn, do đó các khoảng GC cực trị ở nhóm này cần được diễn giải thận trọng.
