...
6. **Implement tasks (loop until done or blocked)**
   ...
   **Milestone verification gate:**
   ...
   2. Determine which verifiers to spawn based on what this task group actually touched:
      ...
      - `spx-api-verifier` — ONLY if this task group modified backend API layer code (controllers, routers, DTOs, OpenAPI specs).
      - `spx-perf-verifier` — ONLY if this task group modified caching, retry/circuit breaker, timeout, connection pool, or observability patterns.
      - `spx-security-verifier` — ONLY if this task group modified auth/authZ, JWT, query construction, file upload, secrets, or security config.
      - `spx-db-verifier` — ONLY if this task group modified migration scripts, entity/model changes, or database query construction.
...
8. **Auto-Verify on Completion**
   ...
   - **Has backend API files**: modified files include API layer code — `.java` files with `@RestController` or `@Controller` (Spring), `@Path` resource classes (Quarkus/JAX-RS), Python files with FastAPI (`from fastapi import`) or DRF (`from rest_framework.views`), API route handlers, DTO/request-response model files, OpenAPI spec files (`openapi.yaml`, `openapi.json`, `swagger.yaml`). Quarkus signals: `Uni<T>`/`Multi<T>` return types, `RestResponse<T>`, `@ServerExceptionMapper`, MicroProfile OpenAPI annotations.
   - **Has reliability/performance patterns**: modified files include caching (`@Cacheable`, `Caffeine`, `@CacheResult`, `cachetools`), retry/circuit breaker (`Resilience4j`, `@Retry`, `@CircuitBreaker`, `@Timeout`, `@Bulkhead`, `@Fallback`, `@RateLimit`, `tenacity`, `pybreaker`), HTTP client calls (`WebClient`, `RestTemplate`, `@RegisterRestClient`, `requests`), connection pool config (`HikariCP`, `Agroal`, `quarkus.datasource.jdbc.*`), or observability patterns (`Micrometer`, `quarkus-smallrye-health`, `@Liveness`, `@Readiness`)
   - **Has security-sensitive code**: modified files include auth/authZ logic (`Spring Security`, `@PreAuthorize`, `@RolesAllowed`, `@Authenticated`, `SecurityIdentity`, `Depends(verify_token)`), JWT handling (`quarkus-oidc`, `quarkus-smallrye-jwt`, `mp.jwt.verify.*`), query construction (including Panache `find()` with string concatenation), file upload, secrets, or CORS/CSRF config (`quarkus.http.cors.*`)
   - **Has database changes**: modified files include migration scripts (Flyway, Liquibase, Alembic, Mongock), ORM entity/model changes (`@Entity`, `@Document`, `PanacheEntity`, `PanacheMongoEntity`, SQLAlchemy models), or raw SQL/MongoDB query construction
   ...
   - `spx-api-verifier` — ONLY if modified files include backend API layer code (controllers, routers, DTOs, OpenAPI specs). Include detected API stack and OpenAPI spec path in verifier instructions.
   - `spx-perf-verifier` — ONLY if modified files include caching, retry/circuit breaker, timeout, connection pool, or observability patterns.
   - `spx-security-verifier` — ONLY if modified files include auth/authZ, JWT, query construction, file upload, secrets, or security config.
   - `spx-db-verifier` — ONLY if modified files include migration scripts, entity/model changes, or database query construction.
...
1. **Select applicable verifiers** — do NOT blindly spawn all verifiers. Decide based on what was actually modified:
   ...
   - `spx-api-verifier`: only if backend API layer code was modified (controllers, routers, DTOs, OpenAPI specs)
   - `spx-perf-verifier`: only if caching, retry/circuit breaker, timeout, connection pool, or observability code was modified
   - `spx-security-verifier`: only if auth/authZ, JWT, query construction, file upload, secrets, or security config was modified
   - `spx-db-verifier`: only if migration scripts, entity/model changes, or database query construction was modified
...
- **Milestone verify gate** — Before starting a new major task group (e.g., moving from 1.x to 2.x tasks), MUST run verification. Spawn verifiers selectively: spx-verifier always, others only if the task group touched relevant areas (architecture, UI files, testable code, backend API code).
- Pause on errors, blockers, or unclear requirements - don't guess
- Use contextFiles from CLI output, don't assume specific file names
- **Auto-verify on completion** — MUST run verification when all AI-doable tasks complete (even if manual/testing tasks remain). Spawn verifiers selectively based on actual files modified: spx-verifier always, spx-arch-verifier only for structural changes, spx-uiux-verifier only if UI files modified, spx-test-verifier only if testable code touched, spx-api-verifier only if backend API layer code modified, spx-perf-verifier only if perf/reliability patterns modified, spx-security-verifier only if security-sensitive code modified, spx-db-verifier only if database changes present.
...
| `spx-api-verifier` | API contract: REST conventions, validation, error handling, versioning, OpenAPI sync |
| `spx-perf-verifier` | Performance & reliability: caching, retry/circuit breaker, timeout, connection pool, observability |
| `spx-security-verifier` | Security: auth/authZ, injection prevention, secrets, input sanitization |
| `spx-db-verifier` | Database: migration safety, query patterns, schema design, backward compatibility |
